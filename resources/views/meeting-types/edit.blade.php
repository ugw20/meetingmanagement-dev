<!-- resources/views/meeting-types/edit.blade.php -->
@extends('layouts.app')

@section('title', 'Edit Jenis Meeting')
...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Form Edit Jenis Meeting</h3>
    </div>
    <form action="{{ route('meeting-types.update', $meetingType) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <div>
                    @php
                        ...
                        $currentFields = $meetingType->required_fields ?? [];
                    @endphp
                    @foreach($allFields as $value => $label)
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="required_fields[]" 
                               value="{{ $value }}" {{ in_array($value, old('required_fields', $currentFields)) ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="custom-control custom-checkbox">
                    <input class="custom-control-input" type="checkbox" id="is_active" name="is_active" value="1" 
                           {{ old('is_active', $meetingType->is_active) ? 'checked' : '' }}>
